...
class StoreSystemStatusRequest extends FormRequest
{
    ...
    {
        return [
            'data.*.type.required' => trans('Loại trạng thái không được để trống.'),
            'data.*.name.required' => trans('Tên không được để trống.'),
            'data.*.name.max' => trans('Trường dữ liệu tối đa :size kí tự.', ['size' => 50]),
            'data.*.note.required' => trans('Mô tả/diễn giải không được để trống.'),
            'data.*.note.max' => trans('Trường dữ liệu tối đa :size kí tự.', ['size' => 100]),
        ];
    }
}
