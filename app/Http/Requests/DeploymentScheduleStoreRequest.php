...
class DeploymentScheduleStoreRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        return [
            'deploymentSchedule.data.*.code' => ['required', 'max:30','unique:deployment_schedules,code','regex:/^[0-9A-Z.\_]+$/'],
            'deploymentSchedule.data.*.title' => ['required', 'max:80'],
            'deploymentSchedule.data.*.status_id' => ['required','numeric',function ($attribute, $value, $fail) {
                if ($value === null) {
                    return;
                }
                if (!is_numeric($value)) {
    ...
    {
        return [
            'deploymentSchedule.data.*.code.required' => trans('Mã lịch triển khai không bỏ trống.'),
            'deploymentSchedule.data.*.code.max' => trans('Mã lịch triển khai không quá 30 ký tự.'),
            'deploymentSchedule.data.*.code.unique' => trans('Mã lịch triển khai đã tồn tại.'),
            'deploymentSchedule.data.*.code.regex' => trans('Mã lịch triển khai phải viết hoa, viết liền, không dấu, không ký tự đặc biệt.'),
